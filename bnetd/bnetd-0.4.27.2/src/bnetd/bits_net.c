...
extern int send_bits_net_ping(t_uint16 dest, t_uint32 qid)
{
    t_packet * p;

    if (!(p = packet_create(packet_class_bits))) {
	eventlog(eventlog_level_error,"send_bits_net_ping","could not create packet");
	return -1;
    }
    packet_set_size(p,sizeof(t_bits_net_ping));
    packet_set_type(p,BITS_NET_PING);
    bits_packet_generic(p,dest);
    bn_int_set(&p->u.bits_net_ping.qid,qid);
    send_bits_packet(p);
    packet_del_ref(p);
    return 0;
}
