 ...
 * Copyright (C) 2003 JEBs@shbe.net
 *
...
static int conn_save_ignore(t_connection * c);
static int conn_load_ignore(t_connection * c);
static void conn_show_new_mail_number(t_connection * c);
...
static void conn_send_welcome(t_connection * c)
{
    ...
    if (c->welcomed)
        return;
    /* Welcome */
    if ((filename = prefs_get_welcomefile()))
    {
        if ((fp = fopen(filename,"r")))
    	{
    	    message_send_file(c,fp);
	        if (fclose(fp)<0)
        		eventlog(eventlog_level_error,"conn_send_welcome","could not close welcome file \"%s\" after reading (fopen: %s)",filename,strerror(errno));
	    }
    	else
	        eventlog(eventlog_level_error,"conn_send_welcome","could not open welcome file \"%s\" for reading (fopen: %s)",filename,strerror(errno));
    }
    /* MOTD */
#ifdef WITH_BITS
    /* Send the network-wide first */
    bits_motd_send(c);
#endif
    handle_command(c, "/motd");
    if (watchlist_load(c)<0)
		eventlog(eventlog_level_error,"conn_send_welcome","could not load watchlist");
    if (friendslist_load(c)<0)
		eventlog(eventlog_level_error,"conn_send_welcome","could not load friendslist");
    if (conn_load_ignore(c)<0)
		eventlog(eventlog_level_error,"conn_send_welcome","could not load ignorelist");
    conn_show_new_mail_number(c);
#ifdef ACCT_DYN_UNLOAD
    account_inc_ref(conn_get_account(c));
#endif
    c->welcomed = 1;
}
...
extern void conn_destroy(t_connection * c)
{
    ...
    if (c->account)
		friends_notify_event(c->account,friends_event_logout,NULL);
    if (c->versioncheck)
    ...
    if (c->welcomed)
    {
		if (watchlist_save(c)<0)
			eventlog(eventlog_level_error,"conn_destroy","can not save watchlist");
		if (friendslist_save(c)<0)
			eventlog(eventlog_level_error,"conn_destroy","can not save friendslist");
		if (conn_save_ignore(c)<0)
			eventlog(eventlog_level_error,"conn_destroy","can not save ignorelist");
	    if (mail_save_number(c)<0)
			eventlog(eventlog_level_error,"conn_destroy","can not save number of mails");
#ifdef ACCT_DYN_UNLOAD
		if (c->account)
			account_dec_ref(c->account);
#endif
    }
    ...
    if (c->account)
    {
		...
		c->account = NULL; /* the account code will free the memory later */ /* FIXME: where exactly? Shouldn'd here be &nodata? XXXXX */
    }
...
extern unsigned char conn_set_loginname(t_connection * c, char const * loginname)
{
    if (!c)
    {
    ...
    if (!loginname)
    {
        eventlog(eventlog_level_error,__FUNCTION__,"got NULL loginname");
        return 0;
    }

    if (c->loginname)
		free((void *)c->loginname); /* avoid warning */
    if (!(c->loginname = strdup(loginname)))
    {
	    eventlog(eventlog_level_error,__FUNCTION__,"[%d] could not allocate memory for c->loginname (%s)",conn_get_socket(c), loginname);
		return 0;
	}
	return 1;
}
...
extern const char * conn_get_loginname(t_connection const * c)
{
    if (!c) {
		eventlog(eventlog_level_error,__FUNCTION__,"got NULL connection");
		return NULL;
...
extern int conn_unget_fake_clienttag(t_connection const * c, char const * tag)
{
    if (!tag)
    {
	eventlog(eventlog_level_error,"conn_unget_fake_clienttag","got NULL tag");
	return -1;
    }
    if (conn_get_clienttag(c)==tag)
	return 0;
    if (account_unget_strattr(tag)<0)
    {
	eventlog(eventlog_level_error,"conn_unget_fake_clienttag","could not unget strattr");
	return -1;
    }
...
extern void conn_set_account(t_connection * c, t_account * account)
{
    ...
    if (account_get_fl_time(c->account)==0)
    {
		...
		account_set_fl_ip(c->account,addr_num_to_ip_str(c->tcp_addr));
    ...
    friends_notify_event(c->account,friends_event_login,NULL);
...
extern int conn_send_ignore(t_connection * c)
{
    ...
    if (!c)
    {
        eventlog(eventlog_level_error,"conn_send_ignore","got NULL connection");
        return -1;
    }
    
    message_send_text(c,message_type_info,c,"You are currently ignoring:");
    if (c->settings.ignore_count == 0)
    {
    ...
    remove_sep = 0;
    for (i = 0; i<c->settings.ignore_count; i++)
	if ((nick = account_get_name(c->settings.ignore_list[i])))
	{
	    eventlog(eventlog_level_error,"asdasd","nick: %s",nick);
    	    if (l+strlen(nick)+2 > (conn_get_class(c) == conn_class_bot ? MAX_MESSAGE_LEN - 13 : MAX_MESSAGE_LEN))
	    {
        	if (remove_sep)
		tstr[l-2] = '\0';
...
static int conn_save_ignore(t_connection * c)
{
    ...
    if (!c)
    {
	eventlog(eventlog_level_error,"conn_save_ignore","got NULL connection");
	return -1;
    }
    
    if (!(ignstr = strdup("")))
    {
	eventlog(eventlog_level_error,"conn_save_ignore","not enought memory to initialize ignstr");
	return -1;
    }
	...
	if (!(nick = account_get_name(c->settings.ignore_list[i])))
	{
	    eventlog(eventlog_level_error,"conn_save_ignore","got NULL nick");
	    continue;
	}
	tptr = ignstr;
	if (i==0)
	{
	    if (!(ignstr = realloc(ignstr,strlen(nick)+1)))
	    {
		eventlog(eventlog_level_error,"conn_save_ignore","could not allocate memory for ignstr");
		account_unget_name(nick);
		free(tptr);
		return -1;
	    }
	    sprintf(ignstr,"%s",nick);
	}
	else
	{
	    if (!(ignstr = realloc(ignstr,strlen(ignstr)+1+strlen(nick)+1)))
	    {
		eventlog(eventlog_level_error,"conn_save_ignore","could not reallocate memmory for ignstr");
		free(tptr);
		account_unget_name(nick);
		return -1;
	    }
	    sprintf(ignstr,"%s,%s",ignstr,nick);
	}
	account_unget_name(nick);
#ifdef ACCT_DYN_UNLOAD
	account_dec_ref(c->settings.ignore_list[i]);
#endif
    }
    if (account_set_strattr(conn_get_account(c),"BNET\\acct\\ignore",ignstr)<0)
    {
	eventlog(eventlog_level_error,"conn_save_ignore","could not set ignorelist");
	free(ignstr);
	return -1;
    }
    free(ignstr);
...
static int conn_load_ignore(t_connection * c)
{
    ...
    {
	eventlog(eventlog_level_error,"conn_load_ignore","got NULL account");
	return -1;
    }
    if (!(tignstr = account_get_ignore(account)))
    {
	eventlog(eventlog_level_error,"conn_load_ignore","got NULL ignore string");
	return -1;
    }
    if (strlen(tignstr)<1)
    {
    ...
    {
	eventlog(eventlog_level_error,"conn_load_ignore","cannot allocate memory for ignstr");
    ...
    {
	if (strcasecmp(ttok,"")==0)
	    continue;
	if ((account = accountlist_find_account(ttok)))
	    conn_add_ignore(c,account);
	else
	    eventlog(eventlog_level_warn,"conn_load_ignore","got NULL ignored account");
    }
    free(pignstr);
...
extern int conn_check_ignoring(t_connection * c, char const * me)
{
    unsigned int i;
    t_account *  temp;
    ...
    if (!me)
    {
	eventlog(eventlog_level_error,"conn_check_ignoring","got NULL me");
	return -1;
    }
    
    if (!(temp = accountlist_find_account(me)))
    {
	eventlog(eventlog_level_error,"conn_check_ignoring","got NULL account");
	return -1;
    }
    if (c->settings.ignore_list)
	for (i=0; i<c->settings.ignore_count; i++)
...
extern int conn_set_channel(t_connection * c, char const * channelname, char const * channelpassword)
{
    if (!c)
    {
	...
	if (conn_get_channel(c)) {
		...
		channel_add_connection(conn_get_channel(c),c,channelpassword);
	    }
	} else {
	    t_query     * q;
	    t_packet    * p;
	...
	{
	    t_channel_loglevel chanlog;

	    chanlog = channel_loglevel_none;
	    if (prefs_get_chanlog())
		    chanlog=channel_loglevel_public|channel_loglevel_command;
	    if (!(c->channel = channel_create(channelname,channelname,NULL,(channel_flags_allowbots | channel_flags_allowopers),chanlog, NULL, NULL, -1, channelpassword)))
	    {
	...
	if (channel_add_connection(conn_get_channel(c),c,channelpassword)<0)
	{
	    if (created)
...
#endif
	conn_send_welcome(c);
	channel_send_topic(c->channel,c);
	friends_notify_event(c->account, friends_event_join_channel,channelname);
    }
...
/*
 * If create (game does not exist):
 *     gamename!=NULL and startver!=0
 * if join (game already exists):
 *     gamename!=NULL, startver==STARTVER_UNKNOWN, and gameinfo=="" (handle_bnet.c)
 * If exit:
 *     gamename==NULL and already in game
 * FIX by KWS
 */
extern int conn_set_game(t_connection * c, char const * gamename, char const * gamepass, char const * gameinfo, t_game_type type, int startver)
{
    int	newgame = 0;
    if (!c)
    {
    ...
    return conn_set_game_bits(c,gamename,gamepass,gameinfo,type,startver,game_option_none);
#else
    if (c->game)
    {
    ...
    if (gamename)
    {
	if (gamelist_find_game(gamename,game_type_all) && strcasecmp(gameinfo,""))
	    return -1;
	if (!(c->game = gamelist_find_game(gamename,type)))
	{
	    /* setup a new game if it does not already exist */
	    if (!(c->game = game_create(gamename,gamepass,gameinfo,type,startver,conn_get_clienttag(c))))
		return -1;
	    friends_notify_event(conn_get_account(c),friends_event_create_game,gamename);
	    newgame = 1;
	...
	if (game_add_player(conn_get_game(c),gamepass,startver,c)<0)
	{
	    /* should never fail for newly created game so no need to call game_destroy() */
	    c->game = NULL; /* bad password or startver # */
	    return -1;
	}
	if (!newgame)
	    friends_notify_event(conn_get_account(c),friends_event_join_game,gamename);
    }
    else
	c->game = NULL;
    return 0;
...
#ifdef WITH_BITS
extern int conn_set_game_bits(t_connection * c, char const * gamename, char const * gamepass, char const * gameinfo, t_game_type type, int startver, t_game_option option)
{
	...
	if (game) {
	    ...
	    send_bits_game_join_request(conn_get_sessionid(c),bits_game_get_id(game),startver);
	    ...
	    send_bits_game_create_request(gamename,gamepass,gameinfo,type,startver,conn_get_sessionid(c),option);
	    friends_notify_event(conn_get_account(c),friends_event_create_game,gamename);
	}
    } else {
...
extern char const * conn_get_chatcharname(t_connection const * c, t_connection const * dst)
{
    ...
    if (!(loginname = conn_get_loginname(c)))
    {
#ifdef ACCT_DYN_LOAD
		eventlog(eventlog_level_error,__FUNCTION__,"got null loginname");
#else
    	eventlog(eventlog_level_error,__FUNCTION__,"[%d] unable to format chatcharname because connection has NULL loginname",conn_get_socket(c));
#endif
        return NULL;
    }
    ...
    /* choose standard (loginname) or realm (charname or *loginname) format */
    if (clienttag && ((strcmp(clienttag, CLIENTTAG_DIABLO2DV) == 0) || (strcmp(clienttag, CLIENTTAG_DIABLO2XP) == 0)))
    {
        if (c->charname)
        {
            if ((chatcharname = malloc(strlen(c->charname) + strlen(loginname) + 2)))
        	sprintf(chatcharname, "%s*%s", c->charname, loginname);
        }
        else
        {
            if ((chatcharname = malloc(strlen(loginname) + 2)))
        	sprintf(chatcharname, "*%s", loginname);
        }
    }
    else
        chatcharname = strdup(loginname);
    
    if (!chatcharname)
    {
		eventlog(eventlog_level_error,__FUNCTION__,"memory allocation failed");
		return "*"; /* should be invalid as a normal user name so we can check below */
    }
...
extern char const * conn_get_playerinfo(t_connection const * c)
{
...
#endif
   {
    ...
    if (conn_unget_fake_clienttag(c,clienttag) < 0)
	    eventlog(eventlog_level_error,"conn_get_playerinfo","could not unget fake client tag");
...
extern int conn_set_playerinfo(t_connection const * c, char const * playerinfo)
{
    ...
    if (strcmp(clienttag,CLIENTTAG_DIABLORTL)==0)
    {
	    ...
	    if (sscanf(playerinfo,"LTRD %u %u %u %u %u %u %u %u %*u",
            &level, &class, &diablo_kills, &strength, &magic, &dexterity, &vitality, &gold)!=8)
	    {
    ...
    else if (strcmp(clienttag,CLIENTTAG_DIABLO2DV)==0)
    {
	    ...
	    eventlog(eventlog_level_trace,__FUNCTION__,"[%d] DIABLO2DV playerinfo request for client \"%s\" playerinfo=\"%s\"",conn_get_socket(c),clienttag,playerinfo);
    }
    else if (strcmp(clienttag,CLIENTTAG_DIABLO2XP)==0)
    {
        /* in playerinfo we get strings of the form "Realmname,charname" */
	    eventlog(eventlog_level_trace,__FUNCTION__,"[%d] DIABLO2XP playerinfo request for client \"%s\" playerinfo=\"%s\"",conn_get_socket(c),clienttag,playerinfo);
    }
    else
    {
...
extern t_connection * connlist_find_connection_by_name(char const * name, char const * realmname)
{
	...
	charname[n] = '\0';
    /* JEBs20020922 Think this shall be "charname" instead of "name", so i changed it */
    return connlist_find_connection_by_charname(charname,temp + 1);
    }
#endif
...
extern int conn_quota_exceeded(t_connection * con, char const * text)
{
	...
	if (con->settings.quota.totcount>=prefs_get_quota_dobae())
	{
	    ...
	    if (con->channel)
		channel_message_log(con->channel,channel_loglevel_public,con,0,"DISCONNECTED FOR DOBAE ABUSE");
	    return 2;
	}
	return 1;
    }
...
extern int conn_set_lastsender(t_connection * c, char const * sender)
{
    ...
    if (!sender)
    {
	eventlog(eventlog_level_error,"conn_set_lastsender","got NULL sender");
	return -1;
    }

    if (!(temp = strdup(sender)))
    {
	eventlog(eventlog_level_error,"conn_set_lastsender","could not allocate memory for temp");
	return -1;
    }
    if (c->lastsender)
	free((void *)c->lastsender); /* avoid warning */
...
static void conn_show_new_mail_number(t_connection * c)
{
    ...
    if (!c)
    {
	eventlog(eventlog_level_error,"conn_show_new_mail_number","got NULL connection");
	return;
    }
    if (!(account = conn_get_account(c)))
    {
	eventlog(eventlog_level_error,"conn_show_new_mail_number","got NULL account");
	return;
    }
    number = mail_number_of_new(account);
    if (number > 0)
    {
	if (number == 1)
	    sprintf(tmsg,"You have 1 new mail");
	else    
	    sprintf(tmsg,"You have %i new mails",number);
	message_send_text(c,message_type_info,c,tmsg);
    }
}

#ifdef ACCT_DYN_UNLOAD
extern unsigned int conn_count_acc_ref(t_account const * account)
{
    t_connection * conn;
    t_elem const * curr;
    ...
    if (!account)
    {
		eventlog(eventlog_level_error,"conn_count_acc_ref","got NULL account");
		return 0;
    }
    ...
    LIST_TRAVERSE_CONST(conn_head,curr)
    {
		...
		if (conn->account == account)
	    	counter++;
		if (conn->settings.ignore_list)
			for (i=0; i<conn->settings.ignore_count; i++)
				if (conn->settings.ignore_list[i]==account)
					counter++;
    }
