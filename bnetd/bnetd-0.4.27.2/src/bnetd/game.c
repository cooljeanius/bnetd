...
#ifdef ACCT_DYN_UNLOAD
		account_dec_ref(game->players[i]);
#endif
    }
    if (game->realmname)
...
static int game_report(t_game * game)
{
    ...
    char const *    tname;
    unsigned int    i;
    unsigned int    realcount, winners, losers, fake_ladder;
...
/* Yoss: why only first disconected player? What if game was 4/4 and 6 of players made discs? */
			if (!disc_set)
			{
...
/* FIXME: Jung-woo fixed this here but we should find out what value results[i] has...
   and why "discisloss" isn't set above in game_result_disconnect */
    ...
    if (fake_ladder)
        fprintf(fp, "[ game is fake ladder ]\n");
...
{
    eventlog(eventlog_level_debug,"game_set_status","changeing status: %s for game: %s (previous: %s)",
	game_get_name(game),game_status_get_str(status),game_status_get_str(game_get_status(game))); /* XXXXXXX */
    if (!game)
    {
...
#endif
{
	...
	{
	    game->ref--;
	    eventlog(eventlog_level_debug,"game_del_player","removing player #%u \"%s\" from \"%s\", %u players left",i,(tname = account_get_name(account)),game_get_name(game),game->ref);
#ifndef WITH_BITS
...
{
    free((void *)players); /* avoid warning */
}
