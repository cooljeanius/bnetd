...
typedef enum
{
    channel_flags_none=0x000,
    channel_flags_public=0x001,
    channel_flags_moderated=0x002,
    channel_flags_restricted=0x004,
    channel_flags_thevoid=0x008,
    channel_flags_system=0x010,      /* Permanent channel + given clienttag = system channel */
    channel_flags_official=0x020,
    channel_flags_permanent=0x040,
    channel_flags_allowbots=0x080,
    channel_flags_allowopers=0x100
} t_channel_flags;

typedef enum
{
    channel_loglevel_none=0,    /* none */
    channel_loglevel_public=1,  /* talking, join, part, etc */
    channel_loglevel_command=2  /* operator command (kick, ban, etc) */
} t_channel_loglevel;
...
#ifdef CHANNEL_INTERNAL_ACCESS
{
    ...
    char const *       password;   /* NULL if no password */
    ...
    t_channel_loglevel loglevel;   /* 0 if not logging */
#ifdef WITH_BITS
...
extern void channel_message_send_to_admins(t_channel const * channel, t_message_type type, t_connection * conn, char const * text);
