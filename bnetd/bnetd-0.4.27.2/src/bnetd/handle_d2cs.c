 ...
 * Copyright (C) 2002          JEBs (JEBs@shbe.net)
 *
...
static int on_d2cs_authreply(t_connection * c, t_packet const * packet)
{
    ...
    /* JEBs20021006 The simple search of the realm by ip prevents us to use multiple D2CS's
     *              on one IP (but different ports).
     *              So i switched it to search by name with a fallback to search by IP,
     *              for those who have wrong settings in realm.list
     */
    if (!(realm=realmlist_find_realm(realmname))) {
        eventlog(eventlog_level_info,"on_d2cs_authreply", "realm not found by name, fallback to search by IP");
        if (!(realm=realmlist_find_realm_by_ip(conn_get_addr(c)))) {
            eventlog(eventlog_level_error,"on_d2cs_authreply", "realm not found");
            return -1;
        }
    }
    if (realm_get_name(realm) && strcasecmp(realmname,realm_get_name(realm))) {
        eventlog(eventlog_level_error,"on_d2cs_authreply", "warn: realm name mismatch %s %s",
...
static int on_d2cs_accountloginreq(t_connection * c, t_packet const * packet)
{
	...
	if (!(account=packet_get_str_const(packet,sizeof(t_d2cs_bnetd_accountloginreq),MAX_USER_NAME))) {
	...
	if (!(client=connlist_find_connection_by_sessionnum(sessionnum))) {
		...
		if (hash_set_str(&passhash,pass_str)<0) {
			...
			if (hash_eq(try_hash,secret_hash)==1) {
				eventlog(eventlog_level_debug,"on_d2cs_accountloginreq","user %s loggedin on d2cs", account);
				reply=BNETD_D2CS_ACCOUNTLOGINREPLY_SUCCEED;
			} else {
				eventlog(eventlog_level_error,"on_d2cs_accountloginreq","user %s hash not match", account);
				reply=BNETD_D2CS_ACCOUNTLOGINREPLY_FAILED;
			}
		}
...
static int on_d2cs_charloginreq(t_connection * c, t_packet const * packet)
{
	...
	if (!(client=connlist_find_connection_by_sessionnum(sessionnum))) {
		...
		free(temp);
		eventlog(eventlog_level_debug,"on_d2cs_charloginreq", "loaded portrait for character %s",charname);
	}
	if ((rpacket=packet_create(packet_class_d2cs_bnetd))) {
