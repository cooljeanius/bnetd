...
#ifdef CONNECTION_INTERNAL_ACCESS
{
    ...
    char const *                  host;
	char const *				  loginname; /* Case-sensitive loginname as typed into the clients login screen */
    char const *                  user;
...
extern int conn_set_channel(t_connection * c, char const * channelname, char const * password);
...
extern unsigned int conn_get_in_size(t_connection const * c) PURE_ATTR();
...
extern int conn_check_ignoring(t_connection * c, char const * me) PURE_ATTR();
