...
extern int handle_bot_packet(t_connection * c, t_packet const * const packet)
{
	...
	case conn_state_bot_password:
		...
		/* Before any futher authentication set the connections loginname */
		if(!conn_set_loginname(c,botuser))
		    break;
		...
		if (!bits_master)
		{
		    ...
		    if (conn_set_channel(c,CHANNEL_NAME_CHAT,NULL)<0)
	...
	case conn_state_loggedin:
		...
		else if ((channel = conn_get_channel(c)) && !conn_quota_exceeded(c,linestr))
		{
		    channel_message_log(channel,channel_loglevel_public,c,1,linestr);
