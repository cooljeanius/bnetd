...
extern int handle_bits_packet(t_connection * conn,t_packet const * const packet)
{
	...
	if (conn->bits->type==bits_to_slave) {
		...
		if (packet_get_type(packet)!=BITS_SESSION_REQUEST) {
	    	    eventlog(eventlog_level_warn,"handle_bits_packet","[%d] got garbage before session request (type=0x%04x)",conn_get_socket(conn),packet_get_type(packet));
	       	    return -1;
		}
	    } else if (conn_get_state(conn)==conn_state_bits_auth) {
	    	if (packet_get_type(packet)!=BITS_AUTH_REQUEST) {
	    	    eventlog(eventlog_level_warn,"handle_bits_packet","[%d] got garbage before auth request (type=0x%04x)",conn_get_socket(conn),packet_get_type(packet));
            	    return -1;
		}
	    } else if (conn_get_state(conn)!=conn_state_loggedin) {
		eventlog(eventlog_level_warn,"handle_bits_packet","[%d] got packet in wrong connection state (\"%s\")",conn_get_socket(conn),conn_state_get_str(conn_get_state(conn)));
		return -1;
	    }
	} else {
	    if (conn_get_state(conn)==conn_state_connected) {
		if (packet_get_type(packet)!=BITS_SESSION_REPLY) {
	    	    eventlog(eventlog_level_warn,"handle_bits_packet","[%d] got garbage before session request (type=0x%04x)",conn_get_socket(conn),packet_get_type(packet));
	       	    return -1;
		}
	    } else if (conn_get_state(conn)==conn_state_bits_auth) {
	    	if (packet_get_type(packet)!=BITS_AUTH_REPLY) {
	    	    eventlog(eventlog_level_warn,"handle_bits_packet","[%d] got garbage before auth request (type=0x%04x)",conn_get_socket(conn),packet_get_type(packet));
            	    return -1;
		}
	    } else if (conn_get_state(conn)!=conn_state_loggedin) {
	...
	/* --- check if the packet is for another host --- */
	...
	/* ------------------------------------------------- */
		case BITS_SESSION_REPLY:
		{
			...
			if (bits_status_ok(packet->u.bits_session_reply.status))
			{
				...
				conn->bits->peeraddr = bits_packet_get_src(packet);
				eventlog(eventlog_level_info,"handle_bits_packet","[%d] sessionkey=0x%08x sessionnum=0x%08x peeraddr=0x%04x",conn_get_socket(conn),conn_get_sessionkey(conn),conn_get_sessionnum(conn),conn->bits->peeraddr);
				conn_set_state(conn,conn_state_bits_auth);
			} else {
	...
	/* ------------------------------------------------- */
		...
		{
		    if ((conn_get_state(conn)!=conn_state_bits_auth)&&(bits_get_myaddr()==BITS_ADDR_PEER)) {
			...
			if (packet_get_size(packet) < sizeof(t_bits_auth_reply)) {
		        ...
		        /* This packet is (probably) for this server */
	...
	/* ------------------------------------------------- */
		case BITS_AUTH_REQUEST:
			if ((conn_get_state(conn)!=conn_state_bits_auth)&&((bits_packet_get_src(packet)!=BITS_ADDR_PEER)&&(conn_get_state(conn)!=conn_state_loggedin))) {
			...
			if (conn->bits->type == bits_to_slave) {
			    ...
			    if (!(name = packet_get_str_const(packet,sizeof(t_bits_auth_request),1024))) { /* FIXME: 1024 should be a constant */
				eventlog(eventlog_level_error,"handle_bits_packet","[%d] got malformed BITS_AUTH_REQUEST packet (no name)",conn_get_socket(conn));
			    }
			    ...
			    packet_set_size(p,sizeof(t_bits_master_auth_request));
			    packet_set_type(p,BITS_MASTER_AUTH_REQUEST);

			    q = query_create(bits_query_type_bits_auth);
		   	    query_attach_conn(q,"client",conn);
			    bn_int_set(&p->u.bits_master_auth_request.qid,query_get_qid(q));
			    bn_int_set(&p->u.bits_master_auth_request.ticks,bn_int_get(packet->u.bits_auth_request.ticks));
			    bn_int_set(&p->u.bits_master_auth_request.sessionkey,conn_get_sessionkey(conn));
			    bn_int_set(&p->u.bits_master_auth_request.ip,conn_get_addr(conn));
			    bn_short_set(&p->u.bits_master_auth_request.port,conn_get_port(conn));
			    memcpy(&p->u.bits_master_auth_request.passhash,&packet->u.bits_auth_request.passhash,sizeof(bn_int)*5);
			    packet_append_string(p,name);
			    bits_packet_generic(p,BITS_ADDR_MASTER);
			    send_bits_packet(p);
			    packet_del_ref(p);
			    eventlog(eventlog_level_debug,"handle_bits_packet","[%d] sent BITS_MASTER_AUTH_REQUEST to master server (name=\"%s\")",conn_get_socket(conn),name);
			} else {
				eventlog(eventlog_level_warn,"handle_bits_packet","got BITS_AUTH_REQUEST from master");
			}
			break;
	/* ------------------------------------------------- */
		case BITS_MASTER_AUTH_REPLY:
		{
			...
			eventlog(eventlog_level_error,"handle_bits_packet","got BITS_MASTER_AUTH_REPLY not from master");
			break;
		    }
		    q = query_find_byqid(bn_int_get(packet->u.bits_master_auth_reply.qid));
		    if (!q) {
			eventlog(eventlog_level_warn,"handle_bits_packet","could not find query with qid 0x%08x",bn_int_get(packet->u.bits_master_auth_reply.qid));
			break;
		    } else
	...
	/* ------------------------------------------------- */
		case BITS_MASTER_AUTH_REQUEST:
		{
			...
			packet_set_size(p,sizeof(t_bits_master_auth_reply));
			packet_set_type(p,BITS_MASTER_AUTH_REPLY);
			bits_packet_generic(p,bits_packet_get_src(packet));
			bn_int_set(&p->u.bits_master_auth_reply.qid,bn_int_get(packet->u.bits_master_auth_request.qid));
			bn_short_set(&p->u.bits_master_auth_reply.address,0);

			ip = bn_int_get(packet->u.bits_master_auth_request.ip);
			port = bn_int_get(packet->u.bits_master_auth_request.port);

			if (bits_check_bits_master_auth_request(packet)) {
			    char const * name;

			    name = packet_get_str_const(packet,sizeof(t_bits_master_auth_request),1024); /* FIXME: 1024 should be a constant */
			    /* name was already checked earlier */
			    addr = bits_new_dyn_addr();
			    if (addr==0) {
				bn_byte_set(&p->u.bits_master_auth_reply.status,BITS_STATUS_TOOBUSY);
		                eventlog(eventlog_level_info,"handle_bits_packet","BITS authentication failed for %s on host 0x%04x (recover<0)",addr_num_to_addr_str(ip,port),bits_packet_get_src(packet));
			    } else {
				bits_hostlist_add(addr,name,ip,port);
			    	bn_byte_set(&p->u.bits_master_auth_reply.status,BITS_STATUS_OK);
			    	bn_short_set(&p->u.bits_master_auth_reply.address,addr);
				eventlog(eventlog_level_info,"handle_bits_packet","BITS authentication successful for %s on host 0x%04x (addr=0x%04x)",addr_num_to_addr_str(ip,port),bits_packet_get_src(packet),addr);
			    }
		            } else {
			    bn_byte_set(&p->u.bits_master_auth_reply.status,BITS_STATUS_AUTHFAILED);
			    eventlog(eventlog_level_info,"handle_bits_packet","BITS authentication failed for %s on host 0x%04x",addr_num_to_addr_str(ip,port),bits_packet_get_src(packet));
			}
			send_bits_packet(p);
			packet_del_ref(p);
		    } else {
			eventlog(eventlog_level_warn,"handle_bits_packet","got BITS_MASTER_AUTH_REQUEST although not master");
		    }
		}
		break;
	/* ------------------------------------------------- */
		case BITS_VA_CREATE_REQ:
			...
			if (conn->bits->type == bits_to_slave) {
				...
				username = packet_get_str_const(packet,sizeof(t_bits_va_create_req),MAX_USER_NAME);
	...
	/* ------------------------------------------------- */
		case BITS_VA_LOCK:
		{
			if (bits_uplink_connection!=NULL) {
				...
				packet_append_string(rpacket,packet_get_str_const(packet,sizeof(t_bits_va_lock),MAX_USER_NAME));
				...
				username = packet_get_str_const(packet,sizeof(t_bits_va_lock),MAX_USER_NAME);
			...
			if (!q) {
				...
				if (account == NULL) {
					if (request) {
						...
						packet_append_string(rpacket,packet_get_str_const(packet,sizeof(t_bits_va_lock_ack),MAX_USER_NAME));
						if (bits_status_ok(packet->u.bits_va_lock_ack.status))
						{
							bits_va_locklist_add(client,packet_get_str_const(packet,sizeof(t_bits_va_lock_ack),MAX_USER_NAME),bn_int_get(packet->u.bits_va_lock_ack.uid));
						}
						if (client)
						{
				...
				if (bits_status_ok(packet->u.bits_va_lock_ack.status)) {
					...
					/* add to own locklist */
					bits_va_locklist_add(bits_uplink_connection,packet_get_str_const(packet,sizeof(t_bits_va_lock_ack),MAX_USER_NAME),bn_int_get(packet->u.bits_va_lock_ack.uid));
...
eventlog(eventlog_level_debug,"handle_bits_packet(BITS_VA_LOGINREQ)","JEBsDebug: Direct bfeore conn_get_playerinfo 1");
	...
	/* ------------------------------------------------- */
		case BITS_VA_LOGIN:
		{
			eventlog(eventlog_level_debug,"handle_bits_packet(BITS_VA_LOGIN)","user \"%s\"(#%u) logged in with sessionid 0x%08x",packet_get_str_const(packet,sizeof(t_bits_va_login),MAX_USER_NAME),bn_int_get(packet->u.bits_va_login.uid),bn_int_get(packet->u.bits_va_login.sessionid));
			bits_loginlist_add(bn_int_get(packet->u.bits_va_login.uid),bn_short_get(packet->u.bits_va_login.host),bn_int_get(packet->u.bits_va_login.sessionid),packet->u.bits_va_login.clienttag,bn_int_get(packet->u.bits_va_login.game_addr),bn_short_get(packet->u.bits_va_login.game_port),packet_get_str_const(packet,sizeof(t_bits_va_login),MAX_USER_NAME));
		}
		break;
	/* ------------------------------------------------- */
		case BITS_VA_LOGOUT:
	...
	/* ------------------------------------------------- */
		case BITS_VA_UPDATE_PLAYERINFO:
		{
			...
			playerinfo = packet_get_str_const(packet,sizeof(t_bits_va_update_playerinfo),MAX_PLAYERINFO_STR);
	...
	/* ------------------------------------------------- */
		case BITS_NET_PING:
		{
		    ...
		    packet_set_size(p,sizeof(t_bits_net_pong));
		    packet_set_type(p,BITS_NET_PONG);
		    bits_packet_generic(p,bits_packet_get_src(packet));
		    bn_int_set(&p->u.bits_net_pong.qid,bn_int_get(packet->u.bits_net_ping.qid));
		    send_bits_packet(p);
		    packet_del_ref(p);
		}
		break;
	/* ------------------------------------------------- */
		case BITS_NET_PONG:
		{
		    ...
		    q = query_find_byqid(bn_int_get(packet->u.bits_net_pong.qid));
		    if (q) {
			query_answer(q,packet);
		    } else {
			eventlog(eventlog_level_warn,"handle_bits_packet","[%d] could not find query 0x%08x for PONG",conn_get_socket(conn),bn_int_get(packet->u.bits_net_pong.qid));
		    }
		}
		break;
	/* ------------------------------------------------- */
	...
	/* ------------------------------------------------- */
		case BITS_CHAT_CHANNELLIST_ADD:
		{
			...
			channel = channel_create(name,
			                         shortname, /* shortname */
			                         NULL,      /* clienttag */
                                     (channel_flags_permanent | channel_flags_allowbots | channel_flags_allowopers),    /* flags */
			...
			if (channel) {
				...
				bn_byte_set(&p->u.bits_chat_channel_join_reply.status,BITS_STATUS_CHANNELFULL); /* just for fun ;) */
                    ...
                    channel = channel_create(channelname,
				                             NULL,
				                             NULL,
                                             (channel_flags_allowbots | channel_flags_allowopers),    /* flags */
                                             0,NULL,NULL,0,NULL); /* FIXME: Ross added this as a quick fix... */
