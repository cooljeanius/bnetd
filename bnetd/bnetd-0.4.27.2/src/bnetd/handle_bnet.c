...
extern int handle_bnet_packet(t_connection * c, t_packet const * const packet)
{
	...
	case CLIENT_COUNTRYINFO1:
		...
		eventlog(eventlog_level_debug,"handle_bnet_packet","[%d] COUNTRYINFO1 packet from tzbias=0x%04x(%+d) langstr=%s countrycode=%s country=%s",conn_get_socket(c),tzbias,tzbias,langstr,countrycode,country);
		conn_set_country(c,country);
		conn_set_tzbias(c,tzbias);
	    }
	    break;
	...
	case CLIENT_COUNTRYINFO_109:
		...
		if (bn_int_tag_eq(packet->u.client_countryinfo_109.clienttag,CLIENTTAG_STARCRAFT)==0)
		    ...
		    if (bn_int_tag_eq(packet->u.client_countryinfo_109.clienttag,CLIENTTAG_WARCRAFT3)==0) /* FIXME: what does this value mean? */
                	bn_int_set(&rpacket->u.server_authreq_109.unknown1,SERVER_AUTHREQ_109_UNKNOWN1_WAR3);
		    else
                	bn_int_set(&rpacket->u.server_authreq_109.unknown1,SERVER_AUTHREQ_109_UNKNOWN1_NORMAL);
	...
	case CLIENT_CREATEACCTREQ1:
		...
		eventlog(eventlog_level_info,"handle_bnet_packet","[%d] new account requested (1) for \"%s\"",conn_get_socket(c),username);
		...
		else
		{
		    ...
		    {
    			eventlog(eventlog_level_info,"handle_bnet_packet","[%d] account not created (already exists)",conn_get_socket(c));
	    		bn_int_set(&rpacket->u.server_createacctreply1.result,SERVER_CREATEACCTREPLY1_RESULT_NO); /* FIXME: change to "account already exists" */
	        }
		    else
#endif
	...
	case CLIENT_CREATEACCTREQ2:
		...
		eventlog(eventlog_level_info,"handle_bnet_packet","[%d] new account requested (2) for \"%s\"",conn_get_socket(c),username);
	...
	case CLIENT_CREATEACCTREQ3:
		...
		if (!(username = packet_get_str_const(packet,sizeof(t_client_createacctreq3),UNCHECKED_NAME_STR)))
		{
		    eventlog(eventlog_level_error,"handle_bnet_packet","[%d] got bad CREATEACCTREQ3 (missing or too long username)",conn_get_socket(c));
		    break;
		}

		eventlog(eventlog_level_error,"handle_bnet_packet","[%d] packet not yet supported",conn_get_socket(c));
	    }
	    break;
	...
	case CLIENT_LOGINREQ1:
		...
		if (!(username = packet_get_str_const(packet,sizeof(t_client_loginreq1),MAX_USER_NAME)))
		{
		    eventlog(eventlog_level_error,"handle_bnet_packet","[%d] got bad LOGINREQ1 (missing or too long username)",conn_get_socket(c));
		    break;
		}

		if(!conn_set_loginname(c, username))	/* JEBs20030516 FIXME: Don't know if it needed in CLIENT_LOGINREQ1 too (char*acc client ?) */
		    break;
		...
		/* already logged in */
		if (connlist_find_connection_by_accountname(username) && prefs_get_kick_old_login()==0)
		{
		...
		else
		{
			...
			else
			{
	    		...
	    		eventlog(eventlog_level_notice,"handle_bnet_packet","[%d] \"%s\" logged in from: %s (correct password)",conn_get_socket(c),(tname = conn_get_username(c)),addr_num_to_addr_str(conn_get_addr(c),conn_get_port(c)));
	...
	case CLIENT_LOGINREQ2:
		...
		if (!(username = packet_get_str_const(packet,sizeof(t_client_loginreq2),MAX_USER_NAME)))
		{
	...
	case CLIENT_STATSREQ:
		    ...
		    i++,name_off+=strlen(name)+1)
		{
		    ...
		    for (j=0,key_off=keys_off;
		        ...
		        j++,key_off+=strlen(key)+1)
			if (account &&
			    (strcasecmp(key,"BNET\\acct\\passhash1")==0        ||
			     strcasecmp(key,"BNET\\acct\\firstlogin_ip")==0    ||
			     strcasecmp(key,"BNET\\acct\\lastlogin_ip")==0     ||
			     strcasecmp(key,"BNET\\acct\\firstlogin_cdkey")==0 ||
			     strcasecmp(key,"BNET\\acct\\lastlogin_cdkey")==0))
			{
	...
	case CLIENT_PLAYERINFOREQ:
		...
		if (!(username = packet_get_str_const(packet,sizeof(t_client_playerinforeq),MAX_USER_NAME)))
		{
	...
	case CLIENT_JOINCHANNEL:
		...
		if (found && conn_set_channel(c,cname,NULL)<0)
	...
	case CLIENT_MESSAGE:
		...
		else if ((channel  = conn_get_channel(c)) && !conn_quota_exceeded(c,text))
		{
		    channel_message_log(channel,channel_loglevel_public,c,1,text);
	    	    /* we don't log here all commands */
	...
	case CLIENT_GAMELISTREQ:
		...
		else /* list all public games of this type */
		{
			...
			{
			    eventlog(eventlog_level_debug,"handle_bnet_packet","[%d] not listing because game is not open",conn_get_socket(c));
			    continue;
			}
	...
	case CLIENT_GAME_REPORT:
		     ...
		     i++,result_off+=sizeof(t_client_game_report_result),player_off+=strlen(player)+1)
		{
		    ...
		    if (!(player = packet_get_str_const(packet,player_off,MAX_USER_NAME)))
		    {
	...
	case CLIENT_LEAVECHANNEL:
	    /* If this user is in a channel, notify everyone that the user has left */
	    if (conn_get_channel(c))
			conn_set_channel(c,NULL,NULL);
	    break;
	...
	case CLIENT_LADDERSEARCHREQ:
		...
		if (!(playername = packet_get_str_const(packet,sizeof(t_client_laddersearchreq),MAX_USER_NAME)))
		{
