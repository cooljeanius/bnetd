...
extern int irc_welcome(t_connection * conn)
{
    ...
    if ((strlen(server_get_name())+7+strlen(BNETD_VERSION)+9+1)<=MAX_IRC_MESSAGE_LEN)
        sprintf(temp,"%s bnetd-" BNETD_VERSION " aroO Oon",server_get_name()); /* FIXME: be honest about modes :) */
    else
        sprintf(temp,":Maximum length exceeded");
