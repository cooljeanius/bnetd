...
static t_query_rc query_handler_bot_loginreq(t_query *q, t_query_reason reason)
{
	...
	if (!bits_status_ok(reply->u.bits_va_loginreply.status)) {
	    ...
	    if (conn_set_channel(client,CHANNEL_NAME_CHAT,NULL)<0)
		    conn_set_channel(client,CHANNEL_NAME_BANNED,NULL); /* should not fail */
	}
	return query_rc_delete;
    default:
	return query_rc_unknown;
...
static t_query_rc query_handler_bits_auth(t_query *q, t_query_reason reason)
{
    ...
    switch (reason) {
	case query_reason_condition:
	    send_bits_auth_reply(client,bn_byte_get(p->u.bits_master_auth_reply.status),bn_short_get(p->u.bits_master_auth_reply.address));
	    if (bits_status_ok(p->u.bits_master_auth_reply.status)) {
	    	bits_ext_set_peeraddr(client,bn_short_get(p->u.bits_master_auth_reply.address));
    	    	conn_set_state(client,conn_state_loggedin);
	    	eventlog(eventlog_level_info,"query_handler_bits_auth","[%d] BITS client logged in with address 0x%04x",conn_get_socket(client),bn_short_get(p->u.bits_master_auth_reply.address));
	    	bits_setup_client(client);
	    } else {
	    	eventlog(eventlog_level_info,"query_handler_bits_auth","[%d] BITS client failed to login with error code 0x%02x",conn_get_socket(client),bn_byte_get(p->u.bits_master_auth_reply.status));
	    }
	    return query_rc_delete;
	case query_reason_error:
	case query_reason_timeout:
	    eventlog(eventlog_level_error,"query_handler_bits_auth","[%d] closing connection (timed out)",conn_get_socket(client));
	    conn_set_state(client,conn_state_destroy);
	    return query_rc_delete;
	default:
	    return query_rc_unknown;
    }
...
/* ------------------------------------------------------------------- */
static t_query_rc query_handler_bits_ping(t_query *q, t_query_reason reason)
{
    t_connection *client;
    t_packet *p;
    int addr;
    ...
    client = query_get_attachment_conn(q,"client");
    addr = query_get_attachment_num(q,"addr");
    ticks = query_get_attachment_num(q,"ticks");
    p = query_get_attachment_packet(q,"reply");
    if (!client) {
	eventlog(eventlog_level_error,"query_handler_bits_ping","{%d} client is NULL",query_get_qid(q));
	return query_rc_error;
    }
	...
	case query_reason_condition:
	    if (!p) {
		eventlog(eventlog_level_error,"query_handler_bits_ping","{%d} reply is NULL",query_get_qid(q));
		return query_rc_error;
    	    }
    	    if (bits_packet_get_src(p)!=addr) {
		eventlog(eventlog_level_error,"query_handler_bits_ping","{%d} got PONG from wrong sender",query_get_qid(q));
		return query_rc_error;
    	    }
    	    conn_set_latency(client,get_ticks()-ticks);
	    return query_rc_delete;
	case query_reason_timeout:
	    eventlog(eventlog_level_error,"query_handler_bits_ping","[%d] closing connection (timed out)",conn_get_socket(client));
	    conn_set_state(client,conn_state_destroy);
	    return query_rc_delete;
	default:
	    return query_rc_unknown;
...
extern void bits_query_init(void) {
    ...
    bits_query_type_ping			  = query_register_handler(query_handler_bits_ping, query_condition_wait, 0, prefs_get_bits_ping_timeout(), "bits_query_type_ping");
}
