...
FILE * hexstrm=NULL; /* shared with server.c and udptestsend.c */
char const * preffile=NULL; /* shared with server.c */
...
# endif
...
#ifdef HAVE_GETPID
    eventlog(eventlog_level_always,"main","bnetd version "BNETD_VERSION" process %u",(unsigned int)getpid());
#else
    eventlog(eventlog_level_always,"main","bnetd version "BNETD_VERSION);
#endif
    eventlog(eventlog_level_always,"main","logging event levels: %s",prefs_get_loglevels());
    ...
    if (watchlist_create()<0)
	eventlog(eventlog_level_error,"main","could not create watch list");
    if (friendslist_create()<0)
	eventlog(eventlog_level_error,"main","could not create friends list");
    accountlist_load_default();
    if (accountlist_create()<0)
    {
	eventlog(eventlog_level_error,"main","could not create accountlist");
	return 1;
    }
    ladderlist_create();
    ...
    eventlog(eventlog_level_debug,"main","server_process finished");
  ...
  bits_fail:
    ...
    eventlog(eventlog_level_always,"main","server has shut down");
    if (pidfile)
    {
	if (remove(pidfile)<0)
	    eventlog(eventlog_level_error,"main","could not remove pid file \"%s\" (remove: %s)",pidfile,strerror(errno));
