...
extern void query_clear_connection(t_connection * c)
{
    t_elem * curr;
	...
	if (!query->attachments)
	    continue;
	LIST_TRAVERSE(query->attachments,curra) {
	    t_query_attachment * att = elem_get_data(curra);
	    if ((att->type == query_attachment_type_conn)&&
	    	(att->u.conn == c))
	    	    att->u.conn = NULL;
	}
    }
...
extern void query_clear_account(t_account * account)
{
    t_elem * curr;
	...
	LIST_TRAVERSE(query->attachments,curra) {
	    t_query_attachment * att = elem_get_data(curra);
	    if ((att->type == query_attachment_type_account)&&
	    	(att->u.account == account))
	    	    att->u.account = NULL;
	}
    }
}
