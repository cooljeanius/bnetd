...
extern t_game_type bngreqtype_to_gtype(char const * clienttag, unsigned short bngtype)
{
	...
	switch (bngtype)
	{
/* FIXME: why not game_type_all?  Does the client ask for specific difficulty levels? */
	case CLIENT_GAMELISTREQ_ALL:
...
extern t_game_type bngtype_to_gtype(char const * clienttag, unsigned short bngtype)
{
	...
	switch (bngtype)
	{
        case CLIENT_GAMETYPE_DIABLO2_OPEN_NORMAL_A:
        case CLIENT_GAMETYPE_DIABLO2_OPEN_NIGHTMARE_A:
        case CLIENT_GAMETYPE_DIABLO2_OPEN_HELL_A:
        case CLIENT_GAMETYPE_DIABLO2_OPEN_NORMAL_B:
        case CLIENT_GAMETYPE_DIABLO2_OPEN_NIGHTMARE_B:
        case CLIENT_GAMETYPE_DIABLO2_OPEN_HELL_B:
	    return game_type_diablo2open; /* FIXME: should we distinguish? */
	...
	case CLIENT_GAMETYPE_DIABLO_2:
	case CLIENT_GAMETYPE_DIABLO_3:
	    return game_type_diablo; /* FIXME: any relation to normal,nightmare,hell difficulty? */
	default:
...
extern unsigned short gtype_to_bngtype(t_game_type gtype)
{
    ...
    case game_type_diablo2closed:
        eventlog(eventlog_level_error,"gtype_to_bngtype","don't know how to list Diablo II");
	return 0;
    case game_type_none:
    default:
        eventlog(eventlog_level_error,"gtype_to_bngtype","unknown game type %u",(unsigned int)gtype);
	return 0xffff;
    }
}
...
extern t_game_result bngresult_to_gresult(unsigned int bngresult)
{
    ...
    default:
	eventlog(eventlog_level_error,"bngresult_to_gresult","unknown bnet game result %u",bngresult);
...
extern int game_parse_info(t_game * game, char const * gameinfo)
{
    ...
    if (!game)
    {
	eventlog(eventlog_level_error,"game_parse_info","got NULL game");
	return -1;
    }
    if (!gameinfo)
    {
	eventlog(eventlog_level_error,"game_parse_info","got NULL gameinfo");
	return -1;
    }
...
*/
	...
	{
	    /* D2 closed games are handled by d2cs so we can have only have a generic
             * startgame4 without any info :(, this fix also a memory leak for description
             * allocation
             */
	    game_set_maxplayers(game,8); /* FIXME: correct? */
	...
	else
	{
	    ...
	    {
		eventlog(eventlog_level_error,"game_parse_info","bad gameinfo format (missing difficulty) \"%s\"",gameinfo);
		return -1;
	    }
	    game_set_maxplayers(game,8); /* FIXME: correct? */
    ...
    eventlog(eventlog_level_debug,"game_parse_info","got info \"%s\" \"%s\" \"%s\" \"%s\" \"%s\" \"%s\" \"%s\" \"%s\" \"%s\" \"%s\"", mapsize, maxplayers, speed, maptype, gametype, option, checksum, tileset, player, mapname);
