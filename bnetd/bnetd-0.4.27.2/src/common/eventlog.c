...
static t_eventlog_level currlevel=eventlog_level_command|
                                  eventlog_level_debug|
                                  eventlog_level_info|
                                  eventlog_level_notice|
                                  eventlog_level_warn|
...
extern int eventlog_add_level(char const * levelname)
{
    ...
    if (strcasecmp(levelname,"all")==0)
    {
	currlevel = eventlog_level_command|eventlog_level_trace|eventlog_level_debug|
		    eventlog_level_info|eventlog_level_notice|eventlog_level_warn|
		    eventlog_level_error|eventlog_level_fatal;
	return 0;
    }
    if (strcasecmp(levelname,"command")==0)
    {
	currlevel |= eventlog_level_command;
	return 0;
    }
    ...
    if (strcasecmp(levelname,"notice")==0)
    {
	currlevel |= eventlog_level_notice;
	return 0;
    }
...
extern int eventlog_del_level(char const * levelname)
{
    ...
    if (strcasecmp(levelname,"none")==0)
    {
	/* NOP. Should add all to be symmetrical but whatever :) */
	return 0;
    }
    if (strcasecmp(levelname,"all")==0)
    {
...
extern char const * eventlog_get_levelname_str(t_eventlog_level level)
{
    switch (level)
    {
    case eventlog_level_always:
	return "always";
    case eventlog_level_command:
	return "command";
...
#endif
{
    ...
    if ((!(level&currlevel) && !(level&eventlog_level_always)) || currlevel==eventlog_level_none)
	return;
    if (!eventstrm)
	return;
