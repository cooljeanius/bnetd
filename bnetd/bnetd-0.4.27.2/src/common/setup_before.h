...
/***************************************************************/
/* Debugging options */
/* FIXME: Add more customizable settings of debug to configure */

/* print bad calling locations of account functions */
/*#define DEBUG_ACCOUNT		 undef this to improve performance (Yoss: now it is settable by configure) */

/* make bnchat, etc. print debug messages */
/*#define CLIENTDEBUG		(Yoss: now it settable by configure) */

/* make the routines call xxx_check() and notice
   xxx_purge() calls during list traversal */
/*#define LIST_DEBUG		undef this to improve performance (Yoss: now it settable by configure) */
/*#define HASHTABLE_DEBUG	undef this to improve performance (Yoss: now it settable by configure) */
...
/*#define TESTUNGET 		undef this to improve performance (Yoss: now it settable by configure) */

/* check ladders for impossible combinations when returning them Yoss: now it settable by configure */
/*#define LADDER_DEBUG */ /* (Yoss: now it settable by configure) */
...
#define BNETD_WELCOME_FILE      "conf/bnwelcome.txt"
...
#define BNETD_FORTUNECMD        "" /* normally "/usr/games/fortune" */
...
#define BNETD_VERSION_TIMEDIV 3600 /* 1 hour */
