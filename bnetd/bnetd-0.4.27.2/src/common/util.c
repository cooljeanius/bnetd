...
{
    char *       out;
    unsigned int inpos, i;
    unsigned int outpos;
    /* FIXME: what other characters does Windows not allow? What about controls chars? */
    char const bad[]= { '\0', '%', '/', '\\', ':', '|', '*', '?', '<', '>', '"', '.' };
    ...
    /* if outpos >= len*3+1 then the buffer was overflowed (soem kind of bug above) */
    out[outpos] = '\0';
...
/* convert binary strings into C-like strings "\xff\007abcd\"\r\n" */
...
/* convert C-like strings like "\xff\007abcd\"\r\n" into binary strings */
    ...
    for (inpos=0,outpos=0; inpos<strlen(in); inpos++)
    {
	    ...
	    case 'x':
		{
			...
			if (in[inpos]!='0' &&
			    ...
			    in[inpos]!='8' &&
			    in[inpos]!='9' &&
			    in[inpos]!='a' && in[inpos]!='A' &&
			    in[inpos]!='b' && in[inpos]!='B' &&
			    in[inpos]!='c' && in[inpos]!='C' &&
			    in[inpos]!='d' && in[inpos]!='D' &&
			    in[inpos]!='e' && in[inpos]!='E' &&
			    in[inpos]!='f' && in[inpos]!='F')
			    break;
			temp[i] = in[inpos++];
		    }
		    temp[i] = '\0';
		    ...
		    if (i<1 || num<1 || num>255) /* leave \x00 as-is */
		    {
			out[outpos++] = '\\';
			out[outpos++] = 'x';
	    ...
	    default:
		    ...
		    if (i<1 || num<1 || num>255) /* leave \000 and bas escapes as-is */
		    {
			out[outpos++] = '\\';
			strcpy(&out[outpos],temp);
			outpos += strlen(temp);
