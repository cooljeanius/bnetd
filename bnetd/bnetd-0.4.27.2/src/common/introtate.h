...
/* valid for 0<n<w and w>0 and w=2**z */
/*#define ROTL(x,n,w) (((x)<<(n)) | ((x)>>((w)-(n))))*/

/* valid for 0<=n<w and w>0 and w=2**z */
/*#define ROTL(x,n,w) (((x)<<(n)) | ((x)>>(((-(n))&((w)-1)))))*/

/* valid for 0<=n and w>0 and w=2**z , depends on 2's complement */
#define ROTL(x,n,w) (((x)<<((n)&((w)-1))) | ((x)>>(((-(n))&((w)-1)))))

/* valid for 0<=n and w>0 and w=2**z , uses an ugly conditional */
/* FIXME: and also a bug because it doesn't work on PPC */
/*#define ROTL(x,n,w) (((n)&((w)-1)) ? (((x)<<((n)&((w)-1))) | ((x)>>((w)-((n)&((w)-1))))) : (x))*/
