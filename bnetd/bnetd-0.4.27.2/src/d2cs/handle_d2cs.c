...
# ifdef WITH_MYSQL
#  include "handle_d2cs_db_mysql.h"
# endif
#endif
...
static int on_client_creategamereq(t_connection * c, t_packet * packet)
{
	...
	if (gamelist_find_game(gamename)) {
		...
		if (gq) {
			eventlog(eventlog_level_error, __FUNCTION__, "client %d is already in game queue",conn_get_sessionnum(c));
...
static int on_client_charlistreq(t_connection * c, t_packet * packet)
{
	...
	char const		* charname;
#ifdef WITH_STORAGE_DB
	t_db_result		accchars = {0};	/* Needs to be inited cause mysql header files didn't cleary initialize the connect var */
#else
	char			* path;
#endif
...
#ifdef WITH_STORAGE_DB
			db_free_result(accchars.res);
			db_close(&accchars.mysql);
#else
			p_closedir(dir);
		}
#endif
