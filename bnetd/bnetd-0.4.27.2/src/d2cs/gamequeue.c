...
extern int gqlist_check_creategame(void)
{
	...
	BEGIN_LIST_TRAVERSE_DATA(gqlist_head,gq)
	{
		...
		if (!c) {
			...
			eventlog(eventlog_level_error, __FUNCTION__, "got NULL game queue for client %s",conn_get_account(c));
			gq_destroy(gq);
			continue;
		} else {
			eventlog(eventlog_level_info, __FUNCTION__, "try create game %s for account %s",gq->gamename,conn_get_account(c));
...
extern int gqlist_update_all_clients(void)
{
	...
	BEGIN_LIST_TRAVERSE_DATA(gqlist_head,gq)
	{
		...
		if (!c) {
			...
			n++;
			eventlog(eventlog_level_debug, __FUNCTION__, "update client %s position to %d",conn_get_account(c),n);
