...
extern int d2gs_active(t_d2gs * gs, t_connection * c)
{
	...
	total_d2gs++;
	eventlog(eventlog_level_info, __FUNCTION__, "game server %s (id: %d) actived (%d total)",addr_num_to_addr_str(conn_get_addr(c),
...
extern int d2gs_deactive(t_d2gs * gs, t_connection * c)
{
	...
	total_d2gs--;
	eventlog(eventlog_level_info, __FUNCTION__, "game server %s (id: %d) deactived (%d left)",addr_num_to_addr_str(conn_get_addr(gs->connection),conn_get_port(gs->connection)),gs->id,total_d2gs);
	gs->state=d2gs_state_none;
	gs->connection=NULL;
	gs->active=0;
	gs->maxgame=0;
