 ...
 */
#define CONFFILE_INTERNAL_ACCESS
...
static int conf_type_get_size(t_conf_type type);
...
static int conf_hexstr_set(void * data, char * value)
{
	...
	p=(char * *)data;
	tmp=NULL;
	if (value && !(tmp=unescape_chars(value))) return -1;
	if (*p) free(*p);
	*p=tmp;
	return 0;
}
...
extern int conf_load_file(char const * filename, t_conf_table * conf_table, void * param_data, int datalen)
{
	...
	for (line=1; (buff=file_get_line(fp)); line++) {
		...
		if (strcmp(item[1],"=")) {
			eventlog(eventlog_level_error, "missing '=' in file %s line %d",filename,line);
			free(item);
			continue;
		}
		match=0;
