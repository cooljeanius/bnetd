...
static int on_d2gs_authreply(t_connection * c, t_packet * packet)
{
	...
	if (reply==D2CS_D2GS_AUTHREPLY_SUCCEED) {
		eventlog(eventlog_level_info, __FUNCTION__, "game server %s authed",addr_num_to_ip_str(conn_get_addr(c)));
...
static int on_d2gs_setgsinfo(t_connection * c, t_packet * packet)
{
	t_d2gs			* gs;
	t_packet		* rpacket;
	unsigned int	maxgame, prev_maxgame, currgame,
					gameflag, i;
	...
	eventlog(eventlog_level_info,__FUNCTION__, "change game server %s max game from %d to %d (%d current)",addr_num_to_ip_str(conn_get_addr(c)),prev_maxgame, maxgame, currgame);
	...
	/* FIXME: JEBs20030516 The next line was removed in Darius patch... */
...
static int on_d2gs_creategamereply(t_connection * c, t_packet * packet)
{
	...
	if (result==D2GS_D2CS_CREATEGAME_SUCCEED) {
		...
		eventlog(eventlog_level_info,__FUNCTION__, "game %s created on gs %d",game_get_name(game),conn_get_d2gs_id(c));
		reply=D2CS_CLIENT_CREATEGAMEREPLY_SUCCEED;
	} else if (result==D2GS_D2CS_JOINGAME_GAME_FULL) {
		eventlog(eventlog_level_info,__FUNCTION__, "failed to add %s to game %s on gs %d (game full)",conn_get_charname(client),game_get_name(game),conn_get_d2gs_id(c));
		reply=D2CS_CLIENT_JOINGAMEREPLY_GAME_FULL;
	} else {
		eventlog(eventlog_level_warn,__FUNCTION__, "failed to create game %s on gs %d",game_get_name(game),conn_get_d2gs_id(c));
		game_destroy(game);
...
static int on_d2gs_joingamereply(t_connection * c, t_packet * packet)
{
	...
	if (result==D2GS_D2CS_JOINGAME_SUCCEED) {
		eventlog(eventlog_level_info, __FUNCTION__, "added %s to game %s on gs %d",conn_get_charname(client),
			game_get_name(game),conn_get_d2gs_id(c));
		reply=D2CS_CLIENT_JOINGAMEREPLY_SUCCEED;
	} else {
		eventlog(eventlog_level_info, __FUNCTION__, "failed to add %s to game %s on gs %d",conn_get_charname(client),
			game_get_name(game),conn_get_d2gs_id(c));
