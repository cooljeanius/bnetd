...
extern int main(int argc, char * argv[])
{
	...
	case 2: /* delete all by entry, destroy */
	    if (rand()%2 || (hashtable_get_length(table)<GOAL && rand()%3))
		break;
	...
	case 5:
		...
		if (hashtable_entry_release(curr)<0)
		{
		    fprintf(stderr,"%s: unable to release entry\n",argv[0]);
		    abort();
		}
	...
	case 7:
	    if (hashtable_get_length(table)<1)
		break;
	    {
		unsigned int pos;
	...
	case 15:
	    printf("count=%u length=%u\n",count,hashtable_get_length(table));
	}
