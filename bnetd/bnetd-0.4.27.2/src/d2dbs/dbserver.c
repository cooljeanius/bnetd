...
t_list * dbs_server_connection_list = NULL; /* shared with dbspacket.c */

static int		dbs_packet_gs_id = 0;
static t_preset_d2gsid	*preset_d2gsid_head = NULL;
static int	dbs_server_listen_socket=-1;
...
static void dbs_server_loop(int ListeningSocket);
static int dbs_server_setup_fdsets(fd_set * pReadFDs, fd_set * pWriteFDs, fd_set * pExceptFDs, int ListeningSocket);
static BOOL dbs_server_read_data(t_d2dbs_connection* conn);
static BOOL dbs_server_write_data(t_d2dbs_connection* conn);
static int dbs_server_list_add_socket(int sd, unsigned int ipaddr);
...
extern int dbs_server_main(const char* pcAddress, unsigned short nPort)
{
...
static int dbs_server_setup_fdsets(t_psock_fd_set * pReadFDs, t_psock_fd_set * pWriteFDs, t_psock_fd_set * pExceptFDs, int lsocket)
{
	t_elem const * elem;
...
extern int dbs_server_shutdown_connection(t_d2dbs_connection* conn)
{
